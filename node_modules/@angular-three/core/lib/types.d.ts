...
export declare type AnyFunction<TObject> = () => TObject;
export declare type AnyConstructor<TObject> = new (...args: any[]) => TObject;
export declare type AnyExtenderFunction<TObject> = (object: TObject) => void;
export declare type Tail<T extends any[]> = T extends [infer A, ...infer R] ? R : never;
...
export declare type NgtTriplet = [x: number, y: number, z: number];
export declare type NgtQuad = [x: number, y: number, z: number, w: number];
...
export declare type NgtVector2 = THREE.Vector2 | Parameters<THREE.Vector2['set']> | Parameters<THREE.Vector2['setScalar']>[0];
export declare type NgtVector3 = THREE.Vector3 | Parameters<THREE.Vector3['set']> | Parameters<THREE.Vector3['setScalar']>[0];
export declare type NgtVector4 = THREE.Vector4 | Parameters<THREE.Vector4['set']> | Parameters<THREE.Vector4['setScalar']>[0];
export declare type NgtColor = THREE.ColorRepresentation;
export declare type NgtColorArray = typeof THREE.Color | Parameters<THREE.Color['set']>;
...
export declare type NgtCameraOptions = NgtCamera | Overwrite<Partial<THREE.PerspectiveCamera>, NgtCommonParameters> | Overwrite<Partial<THREE.OrthographicCamera>, NgtCommonParameters>;
...
export declare type NgtAnimationCallback<TObject> = (state: NgtRender, obj: TObject) => void;
...
export declare type NgtSupportedEvents = 'click' | 'contextmenu' | 'dblclick' | 'wheel' | 'pointerdown' | 'pointerup' | 'pointerleave' | 'pointermove' | 'pointercancel' | 'pointermissed' | 'lostpointercapture';
...
export declare type FilterFunction = (items: THREE.Intersection[], state: NgtRender) => THREE.Intersection[];
export declare type ComputeOffsetsFunction = <TEvent = unknown>(event: TEvent, state: NgtRender) => {
    offsetX: number;
    offsetY: number;
};
...
export declare type NgtSceneOptions = Overwrite<Partial<Omit<THREE.Scene, 'isScene' | 'onBeforeRender' | 'onAfterRender' | 'type'>>, NgtCommonParameters>;
...
export declare type NgtInstance = THREE.Object3D & {
    __ngt?: NgtInstanceInternal;
};
export interface NgtState {
    ...
    size: NgtSize;
    viewport: NgtViewport & {
        getCurrentViewport: (camera?: NgtCamera, target?: THREE.Vector3, size?: NgtSize) => NgtCurrentViewport;
    };
...
export declare type NgtLoaderResult<T> = T extends any[] ? NgtLoader<T[number]> : NgtLoader<T>;
